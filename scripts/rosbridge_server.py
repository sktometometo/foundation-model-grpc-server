...
class Node:
    def __init__(self):
        ...
        self.srv_ic = rospy.Service("~image_captioning", ImageCaptioning, self.handler_ic)
        self.srv_vqa = rospy.Service(
            "~visual_question_answering", VisualQuestionAnswering, self.handler_vqa
        )
    ...
    def handler_vqa(self, req):
        ...
        grpc_request = lavis_server_pb2.VisualQuestionAnsweringRequest(question=req.question)
        grpc_request.image.CopyFrom(cv_array_to_image_proto(cv_array_rgb))
        result = self.stub.VisualQuestionAnswering(grpc_request, wait_for_ready=True)

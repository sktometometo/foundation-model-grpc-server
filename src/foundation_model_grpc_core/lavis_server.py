...
class LAVISServer(LAVISServerServicer):
    ...
    def ImageCaptioning(self, request, context):
        ...
        if self.log_directory is not None:
            ...
            self.save_data(
                ...
                {"caption": caption, "raw_caption": raw_caption},
            )
        if self.use_gui:
    ...
    def InstructedGeneration(self, request, context):
        ...
        if self.log_directory is not None:
            current_datetime = datetime.datetime.now().isoformat()
            self.save_data(
                self.log_directory + "/{}_instructed_generation.png".format(current_datetime),
                cv_array_bgr,
            )
            self.save_data(
                self.log_directory + "/{}_instructed_generation.yaml".format(current_datetime),
                {"instruction": prompt, "caption": "{}".format(response)},
            )
        if self.use_gui:
    ...
    def TextLocalization(self, request, context):
        ...
        if self.log_directory is not None:
            current_datetime = datetime.datetime.now().isoformat()
            self.save_data(
                self.log_directory + "/{}_text_localization_input.png".format(current_datetime),
                cv_array_bgr,
            )
            self.save_data(
                self.log_directory
            ...
            self.save_data(
                self.log_directory + "/{}_text_localization.yaml".format(current_datetime),
                {"text": request.text},
            )
        if self.use_gui:
    ...
    def VisualQuestionAnswering(self, request, context):
        ...
        if self.log_directory is not None:
            current_datetime = datetime.datetime.now().isoformat()
            self.save_data(
                self.log_directory + "/{}_vqa_input.png".format(current_datetime), cv_array_bgr
            )
            cv2.putText(
                cv_array_bgr,
                f"question: {question}\nanswer: {raw_answer}",
            ...
            self.save_data(
                self.log_directory + "/{}_vqa.png".format(current_datetime), cv_array_bgr
            )
            self.save_data(
...
def main_server():
    ...
    parser.add_argument("--use-gui", action="store_true", help="Show GUI Windows if set")
    ...
    if (
        args.device_instructed_generation is not None
        and args.model_instructed_generation is not None
    ):
    ...
    if (
        args.device_visual_question_answering is not None
        and args.model_visual_question_answering is not None
    ):

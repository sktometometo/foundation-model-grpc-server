...
def image_proto_to_cv_array(image: lavis_server_pb2.Image) -> np.ndarray:
    ...
    if image.encoding == cv2.CV_8UC3:
        cv_array = np.frombuffer(image.image_data, dtype=np.uint8).reshape(
        ...
        cv_array = np.frombuffer(image.image_data, dtype=np.float32).reshape(
...
def cv_array_to_image_proto(cv_array: np.ndarray) -> lavis_server_pb2.Image:
    ...
    if dtype == np.uint8 and channels == 3:
        ...
        raise ValueError("Unknown OpenCV encoding. dtype: {}, channel: {}".format(dtype, channels))
    return image
